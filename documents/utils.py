...
class LLMLogger:
    ...
    def log_request(prompts, parsed_reply: Dict[str, Dict]):
        # Ensure LOG_OUTPUT_FILE_PATH is a Path object
        log_output_path = global_config.LOG_OUTPUT_FILE_PATH
        if log_output_path is None:
            # Default to current directory if LOG_OUTPUT_FILE_PATH is not set
            log_output_path = Path(".")
        elif isinstance(log_output_path, str):
            log_output_path = Path(log_output_path)
        
        calls_log = log_output_path / "open_ai_calls.json"
